...
CLT<-function(distr, breaks, params, nsims){
	...
	yydens<-switch(distr,
		...
		rweibull = dweibull(xxdens<-seq(0, params[2]*gamma(1+1/params[1])+20*(params[2]*(gamma(1+2/params[1])-gamma(1+1/params[1]))), length.out=100), shape=params[1], scale=params[2])
	)
